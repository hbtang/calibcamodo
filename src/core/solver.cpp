...
void SolverOrb::OptimizeSclam() {
    ...
    // Add Parameters
    int idParamCamera = 0;
    AddCamPara(optimizer, mpDatasetOrb->mCamMatrix, idParamCamera);

    int idVertexMax = 0;
    // Add camera extrinsic vertex
    int idVertexCamOffset = idVertexMax++;
    AddVertexSE3(optimizer, toG2oIsometry3D(mSe3cb), idVertexCamOffset, false);
    cerr << "mSe3cb.init = " << GetSe3cb() << endl;
    ...
    mSe3cb = toSe3(pVertexCamOffset->estimate());
    cerr << "mSe3cb.opt = " << GetSe3cb() << endl;
