...
int ORBmatcher::MatchByBow(PtrKeyFrameOrb pKF1, PtrKeyFrameOrb pKF2, map<int, int> &mapMatches12) {

    mapMatches12.clear();

    vector<cv::KeyPoint> vKeysUn1 = pKF1->mvecKeyPointUndist;
    DBoW2::FeatureVector vFeatVec1 = pKF1->GetFeatureVector();
    cv::Mat Descriptors1 = pKF1->mDescriptor;

    vector<cv::KeyPoint> vKeysUn2 = pKF2->mvecKeyPointUndist;
    DBoW2::FeatureVector vFeatVec2 = pKF2->GetFeatureVector();
    cv::Mat Descriptors2 = pKF2->mDescriptor;
...
//int ORBmatcher::MatchByWindow(PtrKeyFrameOrb pKfOrb1, PtrKeyFrameOrb pKfOrb2,
...
//    int nmatches = 0;
//    vnMatches12 = vector<int>(pKfOrb1->mNumKeyPoint, -1);
...
//    vector<int> vMatchesDistance(pKfOrb2->mNumKeyPoint, INT_MAX);
//    vector<int> vnMatches21(pKfOrb2->mNumKeyPoint, -1);

//    for(int i1 = 0, iend1 = pKfOrb1->mNumKeyPoint; i1 < iend1; i1++){
//        KeyPoint kp1 = pKfOrb1->mvecKeyPointUndist[i1];
//        int level1 = kp1.octave;
//        if(level1 > maxLevel || level1 < minLevel)
//            continue;
//        int minLevel2 = level1-levelOffset>0 ? level1-levelOffset : 0;
//        vector<size_t> vIndices2 = pKfOrb2->GetFeaturesInArea(vbPrevMatched[i1].x, vbPrevMatched[i1].y, winSize, minLevel2, level1+levelOffset);
//        if(vIndices2.empty())
//            continue;

//        cv::Mat d1 = pKfOrb1->mDescriptor.row(i1);
...
//            cv::Mat d2 = pKfOrb2->mDescriptor.row(i2);
...
//                float rot = pKfOrb1->mvecKeyPointUndist[i1].angle - pKfOrb2->mvecKeyPointUndist[bestIdx2].angle;
//                if(rot < 0.0)
//                    rot += 360.f;
//                int bin = round(rot*factor);
//                if(bin == HISTO_LENGTH)
...
//        if(vnMatches12[i1]>=0)
//            vbPrevMatched[i1]=pKfOrb2->mvecKeyPointUndist[vnMatches12[i1]].pt;
...
//            continue;
//        Point2f predictUV = scv::prjcPt2Cam(Config::Kcam, pNewKF->T2World, pMP->getPos());
